...
public class adapter_recyclerview extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public int getItemViewType(int position) {
        try {
            ...
            else if(jsonObject.get("type").equals("2")) return TYPE_2;
            else return TYPE_3;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        switch (viewType){
            case TYPE_2:
                return new MyViewHolder_nopic(LayoutInflater.from(context).inflate(R.layout.item_test1_1_cardview_nopic,parent,false));
            case TYPE_1:
                return new MyViewHolder(LayoutInflater.from(context).inflate(R.layout.item_test1_1_cardview,parent,false));
            default:return new MyViewHolder_three(LayoutInflater.from(context).inflate(R.layout.item_test1_1_cardview_three,parent,false));
        }
    }
    @Override
    public void onBindViewHolder(final RecyclerView.ViewHolder viewHolder, int position) {
        ...
        //没有图片的holder
        if(viewHolder instanceof MyViewHolder_nopic) {
            final MyViewHolder_nopic holder=(MyViewHolder_nopic)viewHolder;
            try {
                JSONObject jsonObject=jsonArray.getJSONObject(position);
                holder.name.setText(jsonObject.getString("name"));//初始化
                holder.detail.setText(jsonObject.getString("detail"));
        ...
        //三张图片的holder
        if(viewHolder instanceof MyViewHolder_three){
            final MyViewHolder_three holder=(MyViewHolder_three)viewHolder;
            try {
                JSONObject jsonObject=jsonArray.getJSONObject(position);
                holder.name.setText(jsonObject.getString("name"));//初始化
                holder.detail.setText(jsonObject.getString("detail"));
                ...
                Glide.with(context)
                        .load(jsonObject.getString("img1"))
                        .apply(options)
                        .into(holder.img_detail1);
                Glide.with(context)
                        .load(jsonObject.getString("img2"))
                        .apply(options)
                        .into(holder.img_detail2);
                Glide.with(context)
                        .load(jsonObject.getString("img3"))
                        .apply(options)
                        .into(holder.img_detail3);
            if (onItemClickListener != null) {
                holder.player.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
    ...
    private class MyViewHolder_nopic extends RecyclerView.ViewHolder{
        ImageView player,head;
        TextView name,detail;
        MyViewHolder_nopic(View itemView) {
            super(itemView);
    ...
    private class MyViewHolder_three extends RecyclerView.ViewHolder{
        ImageView player,img_detail1,img_detail2,img_detail3,head;
        TextView name,detail;
        MyViewHolder_three(View itemView) {
            super(itemView);
            player= itemView.findViewById(R.id.test1_1_img_player);
            img_detail1=itemView.findViewById(R.id.test1_1_img_detail1);
            img_detail2=itemView.findViewById(R.id.test1_1_img_detail2);
            img_detail3=itemView.findViewById(R.id.test1_1_img_detail3);
            head=itemView.findViewById(R.id.test1_1_img_head);
            name=itemView.findViewById(R.id.test1_1_textview_title);
            detail=itemView.findViewById(R.id.test1_1_testview_detail);
        }
